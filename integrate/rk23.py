...
def rk3_adaptive(u0, f, *args, **kwargs):
    ...
    while not done:
        ...
        r1 = f(t, v)
        r2 = f(t + k, v + k * r1)
        r3 = f(t + 0.5 * k, v + 0.25 * k * (r1 + r2))

        # Count number of function evaluations
        fevals += 3

        # Compute RK3 step
        vnew = v + k * (r1 + r2 + 4. * r3) / 6.0
        ...
        knew = k * min(a_max, max(a_min, safety * np.power(1./err, 1./3)))
        # knew = k * (1./err)**(alpha / 3.0) * (1./perr)**(beta / 3.0)
    ...
    print('Explicit RK3: {0:s}'.format(msg[idid]))
